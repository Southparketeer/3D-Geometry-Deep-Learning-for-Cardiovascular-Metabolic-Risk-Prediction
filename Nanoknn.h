...
public:
	...
	inline void findKNNwithW(const CVertexO& v, const int& k, vector<int>& knnidx, vector<double>& knnw);
	inline int  findBestMesh(const CVertexO& v, const double& weight_N, const vector<int>& knnidx);
	double correspondence(vector<CVertexO>& CorrS, vector<CVertexO>& CorrT, double weight_N = 0, double avgTimes = 20);
	inline double calcEnergy(const CVertexO & p1, const CVertexO & p2, const double& weight_N);
	void  weightAveragedeformMesh(CMeshO & mesh, const vector<Mat4, Eigen::aligned_allocator<Mat4>>& X_acc);
	static void copy(CVertexO&a, CVertexO&b){a.P() = b.P(); a.N() = b.N(); a.C() = b.C();};
private:
	PointCloud<double> *_PC;
...
Nanoknn::Nanoknn(CMeshO& _mod)
{
	...
	Concurrency::parallel_for(0, (int)_mod.vert.size(), [&](int k){
		copy(_cvertexo->at(k), _mod.vert.at(k));
	});
...
Nanoknn::Nanoknn(vector<Vet3>& _vet)
{
	...
	Concurrency::parallel_for(0, (int)_vet.size(), [&](int k){
		_cvertexo->at(k).P() = Point3d(_vet.at(k).x(), _vet.at(k).y(), _vet.at(k).z());
		_cvertexo->at(k).N() = Point3d(0,0,1);
	});
...
inline void Nanoknn::buildNano()
{
	assert(_PC == NULL);
	Timer time;
	double globalTime = time.GlobalTime();

	_PC = new PointCloud<double>;
...
inline void Nanoknn::findNN(const CVertexO& v, int& res_idx)
{
	...
	std::vector<double> out_dist_sqr_clos(1);
...
inline void Nanoknn::findKNNwithW(const CVertexO& v, const int& k, vector<int>& knnidx, vector<double>& knnw)
{
	assert(knnw.size() == k);
	...
	for(int i = 0 ; i < k ; i++)
	{
		...
		double d = sqrtf(out_dist_sqr[i]);
		double weight = (1 - (d)/(dmax));
		knnw[i] = weight;
		sum_w +=  weight;
	}
...
inline double Nanoknn::calcEnergy(const CVertexO & p1, const CVertexO & p2, const double& weight_N)
{
	double d = (p1.P() - p2.P()).Norm();
	double n = p1.N().dot(p2.N());
	return d + n * weight_N;
};


inline int  Nanoknn::findBestMesh(const CVertexO& v, const double& weight_N, const vector<int>& knnidx)
{
	double min = FLT_MAX;
	int k = -1;
	for(int i = 0 ; i < knnidx.size(); i++)
	{
		double e =calcEnergy(v, _cvertexo->at(knnidx.at(i)), weight_N);
...
{
	int knn = 4;
	vector<vector<double>> knnw(mesh.vert.size());
	...
	Concurrency::parallel_for(0,(int)mesh.vert.size(), [&](int k){
		...
		mesh.vert.at(k).P() = Point3d(v_avg.x(), v_avg.y(), v_avg.z());
		mesh.vert.at(k).N() = Point3d(v_avg.x(), v_avg.y(), v_avg.z());
	});
}

double  Nanoknn::correspondence(vector<CVertexO>& CorrS, vector<CVertexO>& CorrT, double weight_N, double avgTimes)
{
	double error = 0;
	vector<int> knnidx;
	CorrT.clear();
	...
	CVertexO outlier;
	outlier.P() = Point3d(FLT_MAX, FLT_MAX, FLT_MAX);
	outlier.N() = Point3d(0,0,1);
	...
	double average = error / (double) CorrS.size();

	for(int i = 0 ; i < CorrS.size(); i++ )
	{
		Point3d source_pos = CorrS.at(i).P();
		Point3d source_nor = CorrS.at(i).N();
		Point3d target_pos = CorrT.at(i).P();
		Point3d target_nor = CorrT.at(i).N();
