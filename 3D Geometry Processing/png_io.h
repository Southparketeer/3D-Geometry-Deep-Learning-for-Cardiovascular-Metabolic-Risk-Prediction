...
{
public:
	static void  PNG_to_array_depth(const char * s_name, _Out_cap_c_(w * h) double* depth,  int w, int h, double scale = 1)
	{
		...
		Concurrency::parallel_for( 0, w * h, [&](int k){
			double d = (double)(image[k * 2] * 256 + image[k * 2 + 1]) * scale;
			depth[k] = ( d  > UINT16_MAX || d  < 0 ) ? UINT16_MAX : d;
		});
		return;
	}
	...
	//	{
	//		int dd = depth[bad_pixel.x + bad_pixel.y * width];
	//		if(dd == UINT16_MAX)
	//		{
	//			int step = 2;
	//			if(bad_pixel.x > 0 && bad_pixel.y > 0)
	...
	//				float d = 0;
	//				d += 0.25 * depth[bad_pixel.x + step + bad_pixel.y * width];
	//				d += 0.25 * depth[bad_pixel.x - step + bad_pixel.y * width];
	//				d += 0.25 * depth[bad_pixel.x + (bad_pixel.y + step) * width];
	//				d += 0.25 * depth[bad_pixel.x + (bad_pixel.y - step) * width];
	...
	//					if(i == 0 && j == 0)
	//						return;
	//					else if(depth[bad_pixel.x + i + (bad_pixel.y + j) * width] == 0)
	//					{
	//						depth[bad_pixel.x + i + (bad_pixel.y + j) * width] = depth[bad_pixel.x + bad_pixel.y * width];
	//					}
	//				}
	//			}
	//		}
	...
	static void  depth_array_to_PNG(_Out_cap_c_(w * h) double* depth,  const char * s_name, int w, int h, double scale = 1., float F = 65585, float N = 0, double base = 0)
	{
		std::vector<unsigned char> image(w * h * 2);
		Concurrency::parallel_for( 0, w * h, [&](int k){
			int d = ceil(depth[k] * scale + base);
			if( depth[k]  > F || depth[k] < N ) 
			{
				image[k * 2 + 0] = 0;
				image[k * 2 + 1] = 0;
			}
	...
	static void  RGB_array_to_PNG(vector<Color4b>& img,  const char * s_name, int w, int h)
	{
		std::vector<unsigned char> image(w * h * 4);
		for(int k = 0; k < w * h; k++)
		{
	...
	static void  Normalized_double_RGB_array_to_PNG(double* img,  const char * s_name, int w, int h)
	{
		std::vector<unsigned char> image(w * h * 4);
		Concurrency::parallel_for( 0, w * h, [&](int k){
			image[k * 4 + 0] = (unsigned char)(img[k * 3 + 0] * 255);
			image[k * 4 + 1] = (unsigned char)(img[k * 3 + 1] * 255);
			image[k * 4 + 2] = (unsigned char)(img[k * 3 + 2] * 255);
			image[k * 4 + 3] = 255;
		});		
		std::vector<unsigned char> png;
		unsigned error = lodepng::encode(png, image, w, h, LCT_RGBA, 8);
	...
	static void  Normalized_double_RGB_array_to_PNG(vector<vcg::Point3d>& img,  const char * s_name, int w, int h)
	{
		std::vector<unsigned char> image(w * h * 4);
		Concurrency::parallel_for( 0, w * h, [&](int k){
			image[k * 4 + 0] = (unsigned char)fabs(img[k].X() * 255);
			image[k * 4 + 1] = (unsigned char)fabs(img[k].Y() * 255);
			image[k * 4 + 2] = (unsigned char)fabs(img[k].Z() * 255);
			image[k * 4 + 3] = 255;
		});		
		std::vector<unsigned char> png;
		unsigned error = lodepng::encode(png, image, w, h, LCT_RGBA, 8);
	...
	static void  BYTE_Mark8_array_to_PNG(_Out_cap_c_(w * h) char* img,  const char * s_name, int w, int h)
	{
		std::vector<unsigned char> image(w * h * 4);
		Concurrency::parallel_for( 0, w * h, [&](int k){
			image[k * 4 + 0] = img[k] ? 255 : 0;
			image[k * 4 + 1] = img[k] ? 255 : 0;
			image[k * 4 + 2] = img[k] ? 255 : 0;
			image[k * 4 + 3] = 255;
		});		
		std::vector<unsigned char> png;
		unsigned error = lodepng::encode(png, image, w, h, LCT_RGBA, 8);
